...
**Periodo coperto:** eventi valvola dal 2026-05-03 al 2026-08-16 (ultimo evento con `duration_seconds > 0`); serie di umidità/ET0/pioggia sul periodo `2026-04-26 .. 2026-08-17` (~113 giorni, riga `periodo:` stampata dallo script — vedi §1 per il dettaglio righe/campioni) a 15 minuti (`analysis/umidita.csv`).
...
| `analysis/eventi.csv` (non versionato) | 296 | eventi valvola con `duration_seconds > 0`, dal 2026-05-03 al 2026-08-16, con tag `trigger` |
...
**Decomposizione (colonna 3 → colonna 4 → colonna 2)**: confrontare in sequenza "senza bagnatura" (colonna 3, la vera baseline) con "solo `w_irr`" (colonna 4) e poi col modello esteso completo (colonna 2) isola il contributo di ciascun coefficiente. `w_irr` da solo (colonna 4) è vicino al pareggio sulla **mediana** — la **batte** perfino a 24h (6.69 pp contro 7.48 pp) — ma **peggiora il p90 a ogni orizzonte**, in modo crescente: +26% a 6h (11.21 contro 8.90 pp), +27% a 12h (13.66 contro 10.76 pp), +46% a 24h (20.00 contro 13.68 pp). `w_rain` aggiunge un ulteriore incremento sopra quello di `w_irr` (colonna 4 → colonna 2), ma su 5 delle 6 metriche misurate (mediana e p90 a 6h/12h, p90 a 24h) questo incremento è **più piccolo** di quello già introdotto da `w_irr` da solo; solo sulla mediana a 24h il contributo di `w_rain` è nullo (colonna 2 = colonna 4 = 6.69 pp). In sintesi: è `w_irr`, non `w_rain`, a contribuire di più al peggioramento in coda. Sulla mediana, il modello esteso (colonna 2) batte "senza bagnatura" (colonna 3) solo a 24h (6.69 contro 7.48 pp); a 6h e 12h resta peggiore su ogni metrica.
...
Va notato, per completezza, che la stessa tabella mette in difficoltà anche la costante di produzione stessa: `w_rain=0` (nessun termine di pioggia) batte `w_rain=1.2` su 5 delle 6 metriche misurate — mediana e p90 a 6h/12h, p90 a 24h — con l'unica eccezione della mediana a 24h (3.32 contro 3.44 pp, a favore di 1.2). Questo non cambia il verdetto di fondo: il valore fittato resta peggiore sia di 0 sia di 1.2, soprattutto in coda; è però un'osservazione onesta sulla costante `1.2` stessa, non solo sul candidato che dovrebbe eventualmente sostituirla.
...
1. **`w_irr` resta il candidato migliore per un secondo giro, ma il quadro è misto, non uniformemente positivo**: da solo (§6.1, colonna 4) è vicino al pareggio sulla **mediana** rispetto alla baseline "senza bagnatura", e a 24h la batte (6.69 contro 7.48 pp). Ma **peggiora il p90 a ogni orizzonte testato**, in modo crescente: +26% a 6h (11.21 contro 8.90 pp), +27% a 12h (13.66 contro 10.76 pp), +46% a 24h (20.00 contro 13.68 pp) — "quasi neutro" descrive quindi solo la mediana, non l'intero quadro. È anche fisicamente plausibile in ordine di grandezza (§5: equivalente ~1.52 %/mm, probabilmente sottostimato dall'assunzione di copertura piena dei 40 m², quindi ancora più vicino a `w_rain` di quanto sembri). Non serve cambiare metodo: serve più campione pulito, che crescerà da solo con `mode=auto` (punto 4 sotto).
...
3. Il campione pioggia usato per `w_rain` (n=38) supera la guardia minima di 15 campioni (§5 di `docs/step16_modello_bagnatura.md`) con più margine rispetto al giro precedente di questo script (n=31), ma resta piccolo rispetto a `w_irr` (n=150) e ha la coda relativa più ampia dei due: un fit ancora fragile, a rischio di instabilità con la prossima estrazione di dati.
...
Non si raccomanda di procedere a una fase 2 unica per entrambi i coefficienti (wiring live nel simulatore dello step 15): il backtest esteso non mostra il miglioramento chiaro a 12h richiesto da quella soglia, e il peggioramento in coda che lo spiega non è opera quasi esclusiva di `w_rain` — anzi, su questo punto specifico è `w_irr` a pesare di più (§6.1). Non si raccomanda nemmeno la conclusione più netta "il modello non aiuta": `w_irr` da solo è vicino al pareggio sulla mediana ed è un candidato ragionevole per un secondo giro con più dati puliti, con lo stesso metodo (nessun cambio di metodologia richiesto). `w_rain`, invece, non è pronto: testato onestamente sul bucket che governa (§6.2), perde contro la costante `1.2` già in produzione a ogni orizzonte — un risultato indipendente dalla decomposizione del backtest a 4 colonne — e va rivisto (più campioni, soglia ricalibrata) prima di poter competere di nuovo con la produzione.
